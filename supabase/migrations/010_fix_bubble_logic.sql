-- Fix bubble logic: use smart detection instead of OR
-- Old tournaments: top-4 scored, bubble = 5th (marked with is_bubble = true)
-- New tournaments: top-5 score, bubble = 6th (position = 6, is_bubble is NULL)
-- Smart logic: is_bubble = true OR (is_bubble IS NULL AND position = 6)
...
-- Recreate dependent views (they depend on the above)
...
-- hall_of_shame_stats reads from player_complete_stats, so it updates automatically
